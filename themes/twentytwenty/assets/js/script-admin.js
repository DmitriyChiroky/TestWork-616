jQuery(document).ready(function($) {
    $('#twe_metabox button[name="clear_meta_field"]').on('click', function(event) {
        event.preventDefault();
        $('#date_product').val('');
        $('#type_product').val('')
        $('#remove_listing_image_button').click()
    });

    $('#twe_metabox button[name="submit_post"]').on('click', function(event) {
        event.preventDefault();
        $('#major-publishing-actions input').click()
    });
